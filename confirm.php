...
if(isset($_REQUEST['payment'])){
	if($_REQUEST['payment']=='cash'){
		$transection_id=substr(md5(date('Y-m-d')),-8);
		$_SESSION['transection_id']=$transection_id;
		$insert= "INSERT INTO bhavya_ayojan set user_id='".$_REQUEST['user_id']."', ayojan_id='".$_REQUEST['ayojan_id']."',user_name='".$_REQUEST['user_name']."', category_id='".$_REQUEST['category_id']."',date='".$_REQUEST['date']."', time='".$_REQUEST['time']."', category_name='".$_REQUEST['category_name']."', product_name='".$_REQUEST['product_name']."', product_id='".$_REQUEST['product_id']."', latitude='".$_REQUEST['latitude']."',longitude='".$_REQUEST['longitude']."',status='".$_REQUEST['status']."',admin_amount='".$_REQUEST['admin_amount']."',amount='".$_REQUEST['amount']."',quantity='".$_REQUEST['quantity']."',entry_date='".$_REQUEST['entry_date']."',payment_mode='".$_REQUEST['payment']."',pincode='".$_REQUEST['pincode']."',user_rating='0',vender_rating='0',vendor_feedback='',user_feedback='', address='".addslashes($_REQUEST['address'])."',state='".addslashes($_REQUEST['state'])."', city='".addslashes($_REQUEST['city'])."',transection_id='".$transection_id."' ";
		// print_r($insert);
		$res=mysqli_query($conn,$insert);
		if($res){
	?>
...
if(isset($_REQUEST['payment'])){
	if($_REQUEST['payment']=='cash'){
		$transection_id=substr(md5(date('Y-m-d')),-8);
		$_SESSION['transection_id']=$transection_id;
		$insert= "INSERT INTO  mandal_booked set user_id='".$_REQUEST['user_id']."', mandal_id='".$_REQUEST['mandal_id']."',
		user_name='".$_REQUEST['user_name']."', mandali_name='".$_REQUEST['mandali_name']."', latitude='".$_REQUEST['latitude']."',longitude='".$_REQUEST['longitude']."',status='".$_REQUEST['status']."',amount='".$_REQUEST['amount']."',entry_date='".$_REQUEST['entry_date']."',pay_type='".$_REQUEST['pay_type']."',pincode='".$_REQUEST['pincode']."', address='".addslashes($_REQUEST['address'])."',state='".addslashes($_REQUEST['state'])."', city='".addslashes($_REQUEST['city'])."',transection_id='".$transection_id."';
		// print_r($insert);
		$res=mysqli_query($conn,$insert);
		if($res){
	?>
